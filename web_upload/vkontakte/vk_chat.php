...
switch ($data->type) {
    ...
    case 'message_new':
		...
		//Если сообщение отправлено через кнопку
		if (isset($data->object->payload)) $message = json_decode($data->object->payload)->button;
		...
		else if ($matches[1] === 'key') {
			if (in_array($userid, VK_ADMINS)){
				$colors = ['positive','negative','primary','secondary'];
				$buttons = [];
				foreach (SERVERS as $key => $server){
					$buttons[] = $vk->button('!' . $key, $colors[array_rand($colors)]);
				}
				
				$buttons[] = $vk->button('!все', $colors[array_rand($colors)]);
				
				
				$buttons = array_chunk($buttons, 4, true);//делим на таблицу. если убрать эту строчку, то кнопки будут идти друг за другом (вертикально)
				$keyboard = [
					'one_time' => false, //false = клавиатура будет видна после нажатия, true - скроется. изменяется сразу для всех в беседе.
					'buttons' => $buttons,
				];
				$vk->keyboard($peerid, 'Тестируем клавиатуру', $keyboard);
			} else $vk->send_vk($peerid, '@id' . $userid . ' (Вы) не можете включить клавиатуру');
		}
		...
		*/
			...
			else if ($matches[1] === 'kick') {
				if (in_array($userid, VK_ADMINS)){
					$fwd = $vk->get_forwarded_id($data);
					if ($fwd > 0) {
						$vk->kick($peerid, $fwd);
					} else $vk->send_vk($peerid, 'Необходимо переслать сообщение от пользователя для кика.');
				} else $vk->send_vk($peerid, 'Это админская команда!');
			}
